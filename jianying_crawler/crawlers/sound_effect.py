...
class SoundEffectCrawler(BaseCrawler):
    ...
    def run(self) -> None:
        ...
        if not categories:
            categories = [{"category_id": 10892, "category_key": "", "category_name": "热门"}]
    ...
    def fetch_categories(self) -> list[dict]:
        ...
        data, raw_path = self.parse_response_or_save_raw(response, "sound_effect_panel.json")
    ...
    def crawl_category(self, category_id: str, category_key: str) -> None:
        ...
        while True:
            ...
            raw_name = f"sound_effect_{category_id}_page_{page_index}.json"
            ...
            if next_offset is None or next_offset <= offset:
                self.repo.upsert_state(self.crawler_name, state_key, str(offset))
                break
            self.repo.upsert_state(self.crawler_name, state_key, str(next_offset))
            offset = next_offset
            page_index += 1
    ...
    def fetch_detail(
        ...
        payload = {
            ...
            "items": [
                {
                    "effect_type": int(effect_type or "3"),
                    "id": resource_id,
                    "source": int(source or "1"),
                }
            ],
            "pack_optional": {
                "fav_scene": None,
        ...
        raw_name = f"sound_effect_detail_{resource_id}.json"
        ...
        for item in iter_effect_items(data):
            ...
            self.repo.upsert_item(
                ...
                title=str(common.get("title") or ""),
                effect_type=detail_effect_type,
                panel=self.panel,
                category_id=category_id,
                source_kind="sound_effect_detail",
