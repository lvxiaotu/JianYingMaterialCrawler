...
from .common import extract_categories, extract_common_attr, extract_depend_resources, extract_item_category_ids, get_effect_item_identity, get_has_more, get_next_offset, iter_aigc_items, iter_effect_items
from ..base import BaseCrawler


class TextTemplateCrawler(BaseCrawler):
    crawler_name = "text_template"
    panel = "text-template"
    fallback_category_id = "10577"
    fallback_category_key = "10577"
    aigc_effect_type = 116
    page_size = 50
    ...
    def fetch_categories(self) -> list[dict]:
        ...
        raw_name = "text_template_panel.json"
    ...
    def crawl_category(self, category_id: str, category_key: str) -> None:
        ...
        while True:
            ...
            raw_name = f"text_template_{category_id}_page_{page_index}.json"
            data, raw_path = self.parse_response_or_save_raw(response, raw_name)
    ...
    def fetch_detail(
        ...
        raw_name = f"text_template_detail_{resource_id}.json"
        ...
        for item in iter_effect_items(data):
            ...
            self.repo.upsert_item(
                ...
                panel=self.panel,
                category_id=category_id,
                source_kind="text_template_detail",
    ...
    def fetch_aigc_items(self) -> None:
        payload = {
            "count": self.page_size,
            "effect_type": self.aigc_effect_type,
            "offset": 0,
        }
        response = self.client.post_json(
        ...
        raw_name = "text_template_aigc_page_1.json"
        data, raw_path = self.parse_response_or_save_raw(response, raw_name)
        ...
        for item in iter_aigc_items(data):
            ...
            self.repo.upsert_item(
                ...
                effect_type=effect_type or str(self.aigc_effect_type),
                panel="aigc_text_template",
                source_kind="text_template_aigc",
                raw_list_path=str(raw_path),
                item_json=item,
            )
            self.add_downloads(resource_id, common, item)
    ...
    def add_dependencies(self, parent_resource_id: str, detail_data: dict, raw_path: str) -> None:
        for dep in extract_depend_resources(detail_data):
            ...
            self.repo.upsert_item(
                ...
                title="",
                effect_type="",
                source_kind="text_template_dependency",
    ...
    def fetch_dependency_detail(
        ...
        raw_name = f"text_template_dependency_{resource_id}.json"
        data, detail_raw_path = self.parse_response_or_save_raw(response, raw_name)
        ...
        for item in iter_effect_items(data):
            ...
            self.repo.upsert_item(
                ...
                title=str(common.get("title") or ""),
                effect_type=effect_type,
                source_kind="text_template_dependency_detail",
                parent_resource_id=parent_resource_id,
                raw_list_path=raw_path,
                raw_detail_path=str(detail_raw_path),
                item_json=common,
