...
class MarketingTemplateCrawler(BaseCrawler):
    crawler_name = "marketing_template"
    page_size = 32
    collection_endpoint_key = "replicate_get_collections_marketing"
    list_endpoint_key = "replicate_get_collection_templates_marketing"
    ...
    def run(self) -> None:
        ...
        for collection in collections:
            for nested in collection.get("collections") or [collection]:
                if not isinstance(nested, dict):
                    continue
                collection_id = str(nested.get("id") or "")
                if not collection_id:
                    continue
                self.repo.upsert_category(
                    crawler_name=self.crawler_name,
                    collection_id=collection_id,
                    category_name=str(nested.get("display_name") or nested.get("name") or ""),
                    raw_json_path="",
                )
    ...
    def fetch_collections(self) -> list[dict]:
        ...
        data, raw_path = self.parse_response_or_save_raw(response, "marketing_template_collections.json")

        collections = extract_collection_items(data)
        for collection in collections:
            top_id = str(collection.get("id") or "")
            if top_id:
                self.repo.upsert_category(
                    crawler_name=self.crawler_name,
            ...
            for nested in collection.get("collections") or []:
                if not isinstance(nested, dict):
                    continue
    ...
    def crawl_collection(self, collection_id: str) -> None:
        ...
        while True:
            payload = {
                ...
                "client_rcm_params": "",
                "client_tab": "",
                "daily_feed_recommend_req_index": 4,
            ...
            raw_name = f"marketing_template_collection_{collection_id}_page_{page_index}.json"
            ...
            for template in templates:
                ...
                self.repo.upsert_item(
                    ...
                    collection_id=collection_id,
                    source_kind="marketing_template",
    ...
    def fetch_detail(self, resource_id: str, raw_path: str, collection_id: str) -> None:
        ...
        raw_name = f"marketing_template_detail_{resource_id}.json"
        ...
        for template in iter_template_items(data):
            ...
            self.repo.upsert_item(
                ...
                title=str(template.get("title") or ""),
                collection_id=collection_id,
                source_kind="marketing_template_detail",
